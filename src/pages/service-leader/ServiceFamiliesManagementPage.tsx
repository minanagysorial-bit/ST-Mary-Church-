...
import { 
  ...
  Phone, Sparkles, FolderPlus, Layers, ShieldCheck, Heart, AlertTriangle, Edit2
} from 'lucide-react';
                      ...
                      <div className="pt-3 border-t border-slate-100 space-y-2 animate-fade-in">
                        ...
                        {members.length === 0 ? (
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {members.map(m => (
                              <div key={m.id} className="p-3 bg-white border border-slate-200 rounded-2xl text-xs space-y-1.5 shadow-xs flex items-start justify-between">
                                <div>
                                  <p className="font-bold text-[#00174a] text-sm">{m.full_name}</p>
                                  <p className="text-[11px] text-slate-400">{m.address || 'العنوان غير مسجل'}</p>
                                  {m.phone && <p className="text-[11px] text-slate-500 font-mono">📞 {m.phone}</p>}
                                </div>
                                <button
                                  onClick={() => handleOpenEditMember(m, fam)}
                                  className="text-slate-400 hover:text-[#002366] hover:bg-blue-50 p-1.5 rounded-lg transition-colors shrink-0"
                                  title="تعديل بيانات المخدوم"
                                >
                                  <Edit2 className="w-4 h-4 text-[#002366]" />
                                </button>
                              </div>
                            ))}
                          </div>
